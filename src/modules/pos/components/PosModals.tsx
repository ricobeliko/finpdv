...
export function PaymentModal({
  ...
  const handleFinalize = async () => {
    ...
    if (finalPayments.length === 0) {
      const typedAmount = parseToCents(inputAmount);
      const paymentAmount = typedAmount > 0 ? typedAmount : totalCents;
      finalPayments = [{ method: selectedMethod, amountCents: paymentAmount }];
    }

    const currentTotalPaid = finalPayments.reduce((sum, p) => sum + p.amountCents, 0);
      ...
      <div className="bg-surface w-full max-w-xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-primary text-white px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-white/70">Escolha uma forma de pagamento e pressione ENTER</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                className="bg-slate-800 hover:bg-slate-900 text-white px-5 py-2 rounded-xl font-bold text-xs flex items-center space-x-1"
              >
                <span>+ Lançar [↵]</span>
              </button>
            </div>
        ...
        <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex justify-between items-center">
          ...
          <button
            ...
            className="bg-primary hover:bg-primary-hover text-white px-6 py-2.5 rounded-xl font-bold text-xs shadow transition-all flex items-center space-x-1.5"
          >
            <Check className="w-4 h-4" />
      ...
      <div className="bg-surface w-full max-w-xs rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 bg-amber-50/20 font-mono text-[11px] text-slate-800 space-y-2.5 overflow-y-auto flex-1 border-b border-dashed border-slate-300">
          ...
          <div className="space-y-1 text-right text-[11px]">
            ...
            <div className="flex justify-between text-slate-600">
              <span>PAGO:</span>
              <span>{formatBRL(sale.totalPaidCents || sale.totalCents)}</span>
            </div>
            {sale.changeCents > 0 && (
        ...
        <div className="p-3 bg-slate-50 flex space-x-2">
          <button
            ...
            disabled={isPrinting}
            className="flex-1 bg-slate-800 hover:bg-slate-900 disabled:bg-slate-400 text-white py-2 rounded-xl font-bold text-xs flex items-center justify-center space-x-1.5 shadow transition-all"
          >
          ...
          <button
            ref={nextBtnRef}
            onClick={onClose}
            className="flex-1 bg-primary hover:bg-primary-hover text-white py-2 rounded-xl font-bold text-xs shadow transition-all"
          >
            Próxima [↵]
          </button>
        </div>
